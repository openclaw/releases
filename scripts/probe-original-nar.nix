...
let
  ...
  pkgs = import baseline.inputs.nixpkgs { system = "x86_64-linux"; };
...
assert recoveredSrc.system == "x86_64-linux";
...
assert recoveredSrc.postFetch == transported.postFetch;
{
  ...
  identity = {
    ...
    inherit (recoveredSrc) outPath drvPath name system outputHash outputHashMode stripRoot allowSubstitutes;
    transport = "file://${zip}";
